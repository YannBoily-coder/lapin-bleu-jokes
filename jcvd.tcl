bind pub -	!jcvd	pub_jcvd
bind pub - !aware      pub_aware
bind pub - !copyright      pub_helpaware

### BOT JOKES ;)

### !aware [nick]

proc pub_aware {nick host hand chan arg} {
 ...
 set results {
   ...
   {Quand on sort d'un placenta à l'age de 42 ans et quand on a l'intelligence, le brain, le computer, la mémoire d'un 40 ans mais qui est vide, elle doit se remplir de jour en jour, elle doit sponging, elle doit elle doit prendre comme une éponge, elle doit elle doit...}
   ...
   {Tu sais, en Amérique, on te demande toujours : 'Comment allez-vous ?', 'Comment est la famille ?', 'Est-ce que t'as brossé tes dents ?'. J'en ai marre de ces questions, j'en ai marre }
   {Un mec comme Tintin, je prendrais un mec comme... un jeune... Le mec qui a fait le blond dans la chaussure noire ! [...] Tu prends un Pierre Richard d'il y a dix ans ! Tu comprends ? Avec les cheveux blonds comme ça, c'est un Tintin, ça ! Tu vois }
   {Alors, Replicant, c'est un film que j'ai fait donc..., que Ringo..., enfin, j'ai trouvé le..., enfin, donc, on a trouvé le script. Après, Ringo l'a réécrit, un peu à sa manière. Tu sais comment il est, hein ? Il est trèèèèès over}
   ...
   {Tu sais, il y a Shakespeare qui a été dire -je crois que c'est Shakespeare : 'A great man is a man who can wait pour son opportunité, et la prendre à cet instant, et la garder}
   {Bob Morane, tu mets du Van Damme là-dedans ! Putain, avec des bras, des biceps, du bon acting -je l'ai prouvé dans Replicant. Je peux pleurer, je peux être méchant, je peux tout faire }
   {Replicant, le fou, c'était moi il y a deux, trois ans ! Pas un tueur, mais le fou ! Et maintenant, c'est de nouveau la santé, le Van Damme d'avant, c'est le Replicant. C'est le mec avant de venir en Amérique, le mec pur, la Belgique, jamais bu, jamais fumé, jamais rien... Dans ma vie, je suis passé par des ténèbres, et des montées, et des descentes... Et maintenant, j'ai compris que la solution de tout, c'est calculation of everything. And now, i know what is good and bad for me. And i can help people ! With my movies }
   {Il faut être a l'attention de savoir qu'on existe!}
   {SCRIPT jcvd.tcl - Jean Claude Van Damme Powa ! - By Jarod - Add On Non Officiel de thor.tcl - http://www.cheekybilly.tk - Copyright 2002 Jarod}
  }
 if {$arg != ""} {
   set joueur $arg
  } {
 ...
 puthelp "PRIVMSG $chan :Jean-Claude Van Damme n'est pas sur $chan aujourd'hui, mais il a dit çà..."
...
proc pub_jcvd {nick host hand chan arg} {
	puthelp "NOTICE $nick :\037Aide Jean-Claude Van Damme - Tapez !aware "
	puthelp "NOTICE $nick :\002Pour avoir une phrase Aware aléatoire"
	puthelp "NOTICE $nick :\002Script Jcdv.tcl v1.0 By Jarod - http://www.cheekybilly.tk"
}

### !copyright

proc pub_helpaware {nick host hand chan arg} {
	puthelp "NOTICE $nick :\037Script Jcdv.tcl (Jean-Claude Van Damme"
	puthelp "NOTICE $nick :\002Add-On Non Officiel de Thor.tcl"
	puthelp "NOTICE $nick :\002Copyright 2002 Jarod - http://www.cheekybilly.tk"
}

putlog "Jcvd.tcl v1.0 - Copyright Jarod - Need Thor.tcl"
